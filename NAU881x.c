...
NAU881x_t nau881x;
/*
#define NAU881X_REG_WRITE( reg, val) i2c_bus_write_byte( nau881x->comm_handle, reg, val)
#define NAU881X_REG_READ( reg) i2c_bus_read_byte( nau881x->comm_handle, reg,)
 */
nau881x_status_t  NAU881X_REG_WRITE(uint8_t reg,uint16_t data)
{  
	
	esp_err_t ret=i2c_bus_write_word(nau881x.comm_handle,reg,&data);
	if (ret!=ESP_OK) return NAU881X_STATUS_ERROR;
	return NAU881X_STATUS_OK;
}

uint16_t NAU881X_REG_READ(uint8_t reg)
{  
	uint16_t data=0;
	i2c_bus_read_word(nau881x.comm_handle,reg,&data);
	return data;
}
...
i2c_device_config_t NAU881x_codec_conf={
	    .scl_speed_hz = 100 * 1000,
        .device_address = NAU881X_I2C_ADDRESS,
};


nau881x_status_t NAU881x_FREE()
{
 i2c_master_bus_rm_device(nau881x.comm_handle);
 //free(nau881x);
 return	NAU881X_STATUS_OK;
}


nau881x_status_t NAU881x_CREATE(i2c_master_bus_handle_t bus)
{
	
 //nau881x=(NAU881x_t*)malloc	(sizeof(NAU881x_t));
 //if (nau881x==NULL) return NAU881X_STATUS_INVALID;
 i2c_master_dev_handle_t handle;
 int ret=i2c_master_bus_add_device(bus, &NAU881x_codec_conf,&handle);
 if (ret!= ESP_OK)return NAU881X_STATUS_INVALID;
 nau881x.comm_handle=handle;
 return	NAU881X_STATUS_OK;
}
...
{
    nau881x->_register[NAU881X_REG_SILICON_REV] = NAU881X_REG_READ( NAU881X_REG_SILICON_REV);
