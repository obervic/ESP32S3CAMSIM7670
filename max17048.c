 ...
 * SPDX-License-Identifier: Apache-2.0
 ...
 Victor Oberemok 
 I2C driver updates 10 July 2025
...
i2c_device_config_t max17048_sensor_conf={
	    .scl_speed_hz = 100 * 1000,
        .device_address = MAX17048_I2C_ADDR_DEFAULT,
};

max17048_handle_t max17048_create(i2c_master_bus_handle_t bus, uint8_t dev_addr)
{
   if (dev_addr!=MAX17048_I2C_ADDR_DEFAULT) max17048_sensor_conf.device_address=dev_addr;
   max17048_handle_t res;
    i2c_master_bus_add_device(bus, &max17048_sensor_conf, &res);
    return res;
}
...
esp_err_t max17048_get_ic_version(max17048_handle_t sensor, uint16_t *ic_version)
{
   ...
   return i2c_bus_read_word(sensor,MAX17048_VERSION_REG,ic_version);
...
esp_err_t max17048_get_chip_id(max17048_handle_t sensor, uint8_t *chip_id)
{
    ...
    return i2c_bus_read_byte(sensor, MAX17048_CHIPID_REG, chip_id);
}
...
esp_err_t max17048_reset(max17048_handle_t sensor)
{
    ...
    uint8_t buf[3] = {MAX17048_CMD_REG,0x54, 0x00};
    return i2c_master_transmit(sensor,&buf[0],3,-1);
...
esp_err_t max17048_get_cell_voltage(max17048_handle_t sensor, float *voltage)
{
    ...
    uint16_t result=0;
     esp_err_t ret= i2c_bus_read_word(sensor,MAX17048_VCELL_REG,&result);
...
esp_err_t max17048_get_cell_percent(max17048_handle_t sensor, float *percent)
{
    ...
    if (!max17048_is_ready(sensor)) {
     ...
     uint16_t result=0;
     esp_err_t ret= i2c_bus_read_word(sensor,MAX17048_SOC_REG,&result);
...
esp_err_t max17048_get_charge_rate(max17048_handle_t sensor, float *percent)
{
    ...
    uint16_t result=0;
     esp_err_t ret= i2c_bus_read_word(sensor,MAX17048_CRATE_REG,&result);
...
esp_err_t max17048_get_alert_voltage(max17048_handle_t sensor, float *minv, float *maxv)
{
    ...
    esp_err_t ret = i2c_bus_read_byte(sensor, MAX17048_VALERT_REG, &result);
    if (ret != ESP_OK) {
        return ESP_FAIL;
    }
    *minv = (float)result * 0.02f;                                                     /*!< Unit is 20mV */

    ret = i2c_bus_read_byte(sensor, MAX17048_VALERT_REG + 1, &result);
    if (ret != ESP_OK) {
        return ESP_FAIL;
    }
...
esp_err_t max17048_set_alert_volatge(max17048_handle_t sensor, float minv, float maxv)
{
    ...
    esp_err_t ret = i2c_bus_write_byte(sensor, MAX17048_VALERT_REG, &minv_int);
    if (ret != ESP_OK) {
        return ESP_FAIL;
    }

    ret = i2c_bus_write_byte(sensor, MAX17048_VALERT_REG + 1, &maxv_int);
    if (ret != ESP_OK) {
        return ESP_FAIL;
    }
    return ESP_OK;
...
esp_err_t max17048_get_alert_flag(max17048_handle_t sensor, uint8_t *flag)
{
    ...
    esp_err_t ret = i2c_bus_read_byte(sensor, MAX17048_STATUS_REG, flag);
    if (ret != ESP_OK) {
        return ESP_FAIL;
    }
    *flag = (*flag) & 0x7F;
...
esp_err_t max17048_clear_alert_flag(max17048_handle_t sensor, max17048_alert_flag_t flag)
{
    ...
    status&=flag;
    ret = i2c_bus_write_byte(sensor, MAX17048_STATUS_REG, &status);
    return ret;
}
...
esp_err_t max17048_clear_alert_status_bit(max17048_handle_t sensor)
{
    ...
    uint16_t result=0;
    esp_err_t ret = i2c_bus_read_word(sensor, MAX17048_CONFIG_REG, &result);
    if (ret != ESP_OK) {
        return ESP_FAIL;
    }
    ...
    return i2c_bus_write_word(sensor, MAX17048_CONFIG_REG,&result);
}
...
esp_err_t max17048_get_hibernation_threshold(max17048_handle_t sensor, float *threshold)
{
    ...
    uint8_t result = 0;
    esp_err_t ret = i2c_bus_read_byte(sensor, MAX17048_HIBRT_REG, &result);
    if (ret != ESP_OK) {
        return ret;
    }
...
esp_err_t max17048_set_hibernation_threshold(max17048_handle_t sensor, float threshold)
{
    ...
    esp_err_t ret = i2c_bus_write_byte(sensor, MAX17048_HIBRT_REG, &threshold_int);
    if (ret != ESP_OK) {
        return ESP_FAIL;
    }
    return ESP_OK;
...
esp_err_t max17048_get_active_threshold(max17048_handle_t sensor, float *threshold)
{
    ...
    uint8_t result = 0;
    esp_err_t ret = i2c_bus_read_byte(sensor, MAX17048_HIBRT_REG + 1, &result);
    if (ret != ESP_OK) {
        return ESP_FAIL;
    }
...
esp_err_t max17048_set_active_threshold(max17048_handle_t sensor, float threshold)
{
    ...
    esp_err_t ret = i2c_bus_write_byte(sensor, MAX17048_HIBRT_REG + 1, &threshold_int);
    if (ret != ESP_OK) {
        return ESP_FAIL;
    }
    return ESP_OK;
...
esp_err_t max17048_enter_hibernation_mode(max17048_handle_t sensor)
{
    ...
    uint16_t buf = 0xFFFF;
    return i2c_bus_write_word(sensor, MAX17048_HIBRT_REG, &buf);
}
...
esp_err_t max17048_is_hibernate(max17048_handle_t sensor, bool *is_hibernate)
{
    ...
    uint16_t result=0;
    esp_err_t ret = i2c_bus_read_word(sensor, MAX17048_MODE_REG, &result);
    if (ret != ESP_OK) {
        return ESP_FAIL;
    }
...
esp_err_t max17048_get_reset_voltage(max17048_handle_t sensor, float *voltage)
{
    ...
    uint8_t result = 0;
    esp_err_t ret = i2c_bus_read_byte(sensor, MAX17048_VRESET_REG, &result);
    if (ret != ESP_OK) {
        return ESP_FAIL;
    }
...
esp_err_t max17048_set_reset_voltage(max17048_handle_t sensor, float voltage)
{
    ...
    esp_err_t ret = i2c_bus_write_byte(sensor, MAX17048_VRESET_REG, &voltage_int);
    return ret;
}
...
esp_err_t max17048_set_reset_voltage_alert_enabled(max17048_handle_t sensor, bool enabled)
{
   ...
   uint16_t result=0;
    esp_err_t ret = i2c_bus_read_word(sensor, MAX17048_STATUS_REG, &result);
    if (ret != ESP_OK) {
        return ret;
    }
    ...
    return i2c_bus_write_word(sensor, MAX17048_STATUS_REG, &result);
}
...
esp_err_t max17048_temperature_compensation(max17048_handle_t sensor, float temperature)
{
    ...
    esp_err_t ret = i2c_bus_write_byte(sensor, MAX17048_CONFIG_REG, &temp_int);
    return ret;
}
...
esp_err_t max17048_set_sleep(max17048_handle_t sensor, bool sleep)
{
    ...
    uint16_t result=0;
    esp_err_t ret = i2c_bus_read_word(sensor, MAX17048_CONFIG_REG, &result);    /*!< It is not possible to individually perform read/write operations on address 0x0D */
